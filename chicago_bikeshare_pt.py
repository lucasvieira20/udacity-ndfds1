...
# for data in range(1, data_list[:19]):
#     print(data)
...
for gender in range(len(data_list)):
    if data_list[gender][-2] == "Male":
        male += 1
    elif data_list[gender][-2] == "Female":
        female += 1
...
answer = "Pois tem dados vazios na coluna de genero"
...
# Loop onde faço as atribuições das variaveis min_trip e max
for trip in range(len(trip_duration_list)):

    if int(trip_duration_list[trip]) < min_trip:
        min_trip = int(trip_duration_list[trip])

    if int(trip_duration_list[trip]) > max_trip:
        max_trip = int(trip_duration_list[trip])
...
def mean_calc(def_list, length):
    ...
    mean_result = count/len(def_list)

    return mean_result

# Executando a função para obter o valor da Média
mean_trip = mean_calc(trip_duration_list, len(trip_duration_list))
...
def median_calc(def_list):
    ...
    for item in range(len(def_list)):
        def_list[item] = int(def_list[item])

    # Ordenar a lista
    list_sorted = sorted(def_list)

    # Tamanho da Lista
    list_length = len(def_list)

    # Pegando Indice do Meio
    list_half_index = int(list_length/2)

    # Atribuindo o resultado a uma variável
    result = list_sorted[list_half_index]
